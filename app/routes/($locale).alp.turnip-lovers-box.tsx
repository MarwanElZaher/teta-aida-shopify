...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
    ...
    if (isArabic) {
        return [
            { title: 'تشكيلة عشّاق اللفت | لفت بلمسة بنجر | تيتا عايدة' },
            { name: 'description', content: 'اكتشف تشكيلة عشّاق اللفت من تيتا عايدة — ٤ برطمانات لفت بلمسة بنجر خفيفة ومنعشة… مناسبة لسفرة رمضان وتوصيل داخل القاهرة.' },
            { name: 'keywords', content: 'تشكيلة لفت، لفت بنجر، مخلل لفت رمضان، تيتا عايدة لفت' }
        ];
    }
...
export async function loader({ context }: LoaderFunctionArgs) {
    ...
    const { product } = await storefront.query(PRODUCT_QUERY, {
        variables: { handle: 'turnip-lovers-box' },
    });
...
export default function TurnipLoversALP() {
